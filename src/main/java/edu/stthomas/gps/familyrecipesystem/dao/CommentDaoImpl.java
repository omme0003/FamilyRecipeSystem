...
public class CommentDaoImpl extends AbstractDaoImpl<Comment> implements CommentDao {
	...
	public List<Comment> getCommentsByRecipe(final int recipeId) {
		...
		return session.createQuery("FROM comment WHERE comment.recipeId = ?").setParameter(0, recipeId).list();
	}

	@Override
	public List<Comment> getCommentsByMember(final int memberId) {
		final Session session = this.getSessionFactory().getCurrentSession();
		return session.createQuery("FROM comment WHERE comment.member_id = ?").setParameter(0, memberId).list();
	}
	...
	public Comment getCommentById(final int id) {
		final Session session = this.getSessionFactory().getCurrentSession();
		return (Comment) session.createQuery("FROM comment WHERE comment.id = ?").setParameter(0, id).list().get(0);
	}
